...
│                                                           │
│  User → UI → API → Agent → retrieval → Neon (Postgres)   │
│                          ↓                                │
│                    Gemini (LLM)                           │
│                          ↓                                │
│                   Answer → UI                             │
...
[retrieve_node]   ← embeds question, searches Neon, reranks results
   │
   ▼
[generate_node]   ← calls Gemini with chunks + question → writes the answer
   │
   ▼
[END]
```
...
The Streamlit UI doesn't call the agent directly — it sends HTTP POST requests
to this API. This separation means you could later swap the UI for a mobile app,
a Slack bot, or a curl command without changing the agent.

**Interactive docs:** `http://127.0.0.1:8000/docs` (auto-generated by FastAPI)

---

### `src/fda_rag/ui/` — ONLINE path, the user-facing layer 🔜 Coming Week 6

**What it will do:** A Streamlit web app — the chat interface the user actually sees.

Streamlit turns a plain Python script into a web page. The UI will send the user's
question to the FastAPI endpoint and display the answer + source citations.

---

### `src/fda_rag/eval/` — separate, runs on demand 🔜 Coming Week 7
...
- `download_dailymed.py` — fetches XML files from the DailyMed API (rate-limited)
...
- `data/sample/xml/` — 10 FDA drug label XML files (committed, ~2-4 MB)
- `data/raw/xml/` — full 50-label set (gitignored)
...
scripts/download_dailymed.py
    ...
    data/sample/xml/
         ...
         ▼
fda_rag.ingestion
  ├── parse XML    →  extract 8 section types by LOINC code
  ├── chunk text   →  1500-char windows, 200-char overlap
  ├── embed        →  Voyage AI voyage-3 → float[1024]
  └── store        →  INSERT INTO drug_chunks (text, embedding) VALUES (...)
         │
         ▼
    Neon (Postgres 16 + pgvector)
    HNSW index on embedding column for fast approximate nearest-neighbour search
```
...
LangGraph agent (agent/)
      │
      ├─► [retrieve_node]
      │       embed question → Voyage AI voyage-3 (input_type="query")
      │       SELECT … ORDER BY embedding <=> query_vec LIMIT 20
      │       rerank top-20 → Voyage AI rerank-2 → top 5 chunks
      │
      ├─► [generate_node]
      │       system prompt + 5 chunks + question → Google Gemini → answer
      │
      └─► returns { answer, chunks }
      │
      ▼
FastAPI → JSON response { question, answer, sources }
      │
      ▼
...
| `GEMINI_API_KEY` | `agent/` generate node — calling Google Gemini |
| `LANGSMITH_API_KEY` | LangSmith SDK, auto-picked up from env — traces agent calls |
| `LANGSMITH_PROJECT` | Groups all traces under one project name in the dashboard |
...
agent/
 ├── imports retrieval/
 └── calls Google Gemini API (external)
